...
copier.prototype.copyFile = function(file, dest) {
    ...
    if(this.running >= this.limit) {
        this.scheduleNext(file, dest);
    } else {
        this.streamCp(file, dest);
    }
}

copier.prototype.scheduleNext = function(file, dest) {
    var interval = setInterval(function(file, dest) {
        if(this.running < this.limit) {
            clearInterval(interval);
            this.streamCp(file, dest);
        }
    }.bind(this), 10, file, dest);
}

copier.prototype.streamCp = function(file, dest) {
    var self = this;
    this.running++;
